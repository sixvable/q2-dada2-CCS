...
def _check_featureless_table(fp):
    ...
    if line_count < 2:
        raise ValueError(
            "No features remain after denoising. Try adjusting "
...
_POOL_STR = (lambda x: x in {"pseudo", "independent"}, "pseudo or independent")
_CHIM_STR = (
    lambda x: x in {"pooled", "consensus", "none"},
...
def _check_inputs(**kwargs):
    for param, arg in kwargs.items():
        ...
        if not check_is_valid(arg):
            raise ValueError(
                "Argument to %r was %r, should be %s." % (param, arg, explanation)
            )
...
def _denoise_helper(biom_fp, track_fp, hashed_feature_ids):
    ...
    sid_map = {id_: _filepath_to_sample(id_) for id_ in table.ids(axis="sample")}
...
def _denoise_single(
    ...
    if trunc_len != 0 and trim_left >= trunc_len:
        raise ValueError(
            "trim_left (%r) must be smaller than trunc_len (%r)"
            % (trim_left, trunc_len)
        )
    if max_len != 0 and max_len < trunc_len:
        raise ValueError(
            "trunc_len (%r) must be no bigger than max_len (%r)" % (trunc_len, max_len)
        )
...
def denoise_paired(
    ...
    if trunc_len_f != 0 and trim_left_f >= trunc_len_f:
        raise ValueError(
            "trim_left_f (%r) must be smaller than trunc_len_f"
            " (%r)" % (trim_left_f, trunc_len_f)
        )
    if trunc_len_r != 0 and trim_left_r >= trunc_len_r:
        raise ValueError(
            "trim_left_r (%r) must be smaller than trunc_len_r"
